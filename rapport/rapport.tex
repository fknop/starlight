...
La classe \texttt{ObserverInterface} est suffixé par \texttt{Interface} puisqu'il
s'agit d'une interface (pas au sens C++ car celui-ci ne possède pas la notion
d'interface) car cette classe ne possède que des méthodes \texttt{virtual} pure
qui sont donc obligé d'être redéfini dans une classe héritant de celle-ci.
Cela définit donc parfaitement la notion d'interface.

Cette classe possède une méthode \texttt{notify(...)} qui est apellé à chaque fois
...
Un observé peut ajouter des observateurs à sa liste ou en supprimer gràces aux méthodes :
\begin{lstlisting}
...
Cette méthode apellera pour chaque observateur la méthode \texttt{notify(...)} de ce dernier.
...
ne peut pas savoir qui a envoyé une notification, gràce à ce pointeur, il peut donc agir sur l'objet. De plus, un message
...
de collisions où il a fallu envoyé le déplacement voulu en paramètre pour que le niveau puisse vérifier si ce déplacement est
...
sous-classe est observé par la vue correspondante.
    
\subsubsection{\label{Source}Source}

La classe \texttt{Source} représente la source lumineuse du jeu.
Elle est de forme rectangulaire et peut donc être transformé
...
et peut également être transformé en \texttt{Rectangle} (cf. \hyperref[Rectangle]{''Rectangle''}).
...
une ellipse possédant une intervalle de longueur d'onde. Le rayon doit
être dans cette intervalle pour pouvoir traverser la lentille, dans le cas
...
source et son point de départ, il apelle ensuite compute\_ray(...) qui va chercher
l'intersection la plus proche en apellant get\_intersections(...). Cette dernière méthode
va remplir un vecteur (\texttt{vector}) d'\texttt{Intersection} qui est une structure créé dans 
...
Toutes les intersections seront ensuite triés dans l'ordre croissant selon la distance entre le point \texttt{start} et
...
Les murs sont représententés par des segments de droite noirs.
...
umath::is_on_good_side(const Line& l, 
                       ...
                       const Point& start);
\end{lstlisting}
peut avoir des problèmes de précisions, ce qui fait que l'intersections, bien que du bon côté, ne sera pas 
...
\subsection{\label{AnnexeC}Annexe C : Algorithme simplifié de la logique métier}

\begin{algorithm}
\DontPrintSemicolon
\Begin{
    $droite : Line$
    $droite \longleftarrow Line(source.getPoint(), source.getAngle())$
    $compute\_ray(droite, source.getPoint(), source.getWavelength)$
}
    \caption {compute\_rays()}
\end{algorithm}

\begin{algorithm}
\DontPrintSemicolon
\KwData{$intersections$ : liste d'Intersection // attribut de classe}
\Begin{

$continue : booleen $

$getIntersections(droite, debut)$ // Remplit la liste d'intersections \;
$type : Type$ \;
$type \longleftarrow intersections.get(0).getType()$

\Switch{$type$}
{ 
    \Case{Miroir} 
    {
        $angle \longleftarrow getNewAngle(...)$ \;
        $continue \longleftarrow vrai$
    }
    \Case{Lentille}
    {  
        \eIf{longueur d'onde dans l'intervalle}
        { 
            $continue \longleftarrow vrai$
        }{ 
            $continue \longleftarrow faux$
        }
    }
    \Case{Cristal} 
    { 
        $wl \longleftarrow wl + cristal.getWl()$ \;
        $continue \longleftarrow vrai$
    }
    \Case{Mur} 
    { 
    ...
    { 
        $d : Droite$ \;
        $d \longleftarrow Line(intersections.get(0).getPoint(), angle)$ \;
        $compute\_ray(d, intersections.get(0).getPoint(), wl)$
    }

\caption{compute\_ray(droite : Line, debut : Point, wl : entier)}
}
