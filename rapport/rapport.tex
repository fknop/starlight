...
\item La documentation se fait comme suit :
    \begin{lstlisting}[frame=single]
    /**
     * Description de la methode.
     * @param param1 description param1.
     * @param param2 description param2.
     * @throw exception cas du lancement 
       de l'exception.
     * @return description de ce que la 
       methode retourne.
    */
    \end{lstlisting}
\end{itemize}
...
Cette section décrit les objectifs principaux et secondaires effectués lorssti
...
\subsubsection{Faiblesses de l'interface} 
...
\newpage
\section{\label{Editeur}Éditeur de cartes}
...
\subsection{Possibilités de l'éditeur}
...
Les propriétés sont normalement toutes vérifiées par le modèle pour éviter de donner des données
incohérentes. Un message d'erreur s'affiche donc dans l'éditeur avec la liste
des propriétés ne respectant pas les conditions du modèles quand l'utilisateur
entre des données incohérentes.
...
\subsection{Faiblesses de l'éditeur}

Les déplacements ne se font que au clavier, ce qui peut s'avérer parfois difficile.
...
\newpage
\chapter{Présentation des différentes classes}
...
\section{\label{OO}Observateur / Observé}
...
\subsection{\label{Observable}Observable}
...
\section{Les classes/namespaces utilitaires}
...
\subsection{\label{LineSegment}LineSegment}
...
\subsection{\label{Ellipse}Ellipse}
...
\subsection{\label{Rectangle}Rectangle}
...
\subsection{\label{Source}Source}
...
\subsection{\label{Mirror}Mirror}
...
\subsection{\label{Crystal}Crystal}
...
\begin{lstlisting}[frame=single]
void compute_rays();
void compute_ray(const Line& line,
                 const Point& start,
                 int wl);
...
\subsection{\label{SourceView}SourceView}
...
\subsection{\label{DestinationView}DestinationView}
...
\subsection{\label{WallView}WallView}
...
\subsection{\label{MirrorView}MirrorView}
...
\subsection{\label{LensView}LensView}
...
\subsection{\label{CrystalView}CrystalView}
...
\subsection{\label{NukeView}NukeView}
...
\subsection{\label{RayView}RayView}
...
\subsection{\label{MapView}MapView}
...
\newpage
\section{\label{EditeurClasses}L'éditeur de carte}

\subsection{\label{MainEditor}MainEditor}
...
\subsection{\label{Elements}Elements}
...
\subsection{\label{PropertiesInterface}PropertiesInterface}
...
\subsection{\label{CrystalProp}CrystalProp}
...
\subsection{\label{DestProp}DestProp}
...
\subsection{\label{LensProp}LensProp}
...
\subsection{\label{MirrorProp}MirrorProp}
...
\subsection{\label{NukeProp}NukeProp}
...
\subsection{\label{SourceProp}SourceProp}
...
\subsection{\label{WallProp}WallProp}
...
\newpage
\chapter{\label{Annexe}Annexes}

\section{Annexe A : Démarches mathématiques}

\subsection{\label{AnnexeRotation}Rotation de segment}
...
\newpage
\subsection{\label{AnnexeIntersects}Trouver l'intersection entre deux droites}
...
\subsection{Trouver l'intersection entre une droite et un segment de droite}
...
\subsection{Trouver l'intersection entre deux segments}
...
\subsection{Trouver la ou les intersection(s) entre une ellipse et une droite}
...
Nous pouvons poser quelques variables pour nous faciliter le calcul. 
...
\subsection{Trouver la ou les intersection(s) entre une ellipse et un segment de droite}
...
\subsection{Trouver la ou les intersection(s) entre un rectangle et une droite}
...
\subsection{Trouver la ou les intersection(s) entre un rectangle et un segment de droite}
...
\subsection{Calculer la réflexion d'un rayon}

Nous nous basons ici sur la réflexion d'un rayon sur un miroir plan dans l'air. 
C'est à dire que que l'angle d'incidence sera égal à l'angle réfléchi.
Pour ce faire, nous avons utilisé cette formule :

Soit $\alpha$ l'inclinaison du miroir. \\
Soit $\gamma$ l'angle du rayon émis. \\
Soit $\delta$ l'angle de la perpendiculaire au miroir : $\delta = \alpha + \frac{\pi}{2}$
Soit $\beta$ la différence entre l'angle de la perpendiculaire et l'angle émis : $\beta = \delta - \alpha$
L'angle réfléchi (par rapport à l'axe x) vaut donc : 

$$ \gamma + \pi + (2 * \beta) $$

En effet, lorsque l'on additionne $\pi$ à l'angle émis, on obtient un rayon dans le sens inverse.
Il faut ensuite additionner deux fois l'angle entre la perpendiculaire et le rayon pour obtenir l'angle de réflexion.

Pour démontrer la véracité de cette formule, nous avons fait un graphique ci-dessous. Nous allons ici travailler en degré plutôt qu'en radians.
Nous avons ici un angle émis $\gamma$ de $267,49$ degrés. L'inclinaison du miroir $\alpha$ vaut $30$ degrés.
$\delta$ (perpendiculaire au miroir vaut) $30 + 90$, donc $120$ degrés.
$\beta$ vaut alors : $120 - 267,39 = -147,49$ 

Si nous reprenons la formule de départ : l'angle final vaut : 
$$ 267.49 + 180 + (2 * -147,49) = 152,51 $$ 

\begin{figure}
\input{reflexion.tex}
\caption{Démonstration de la réflexion - Image réalisée à l'aide de Geogebra}
\end{figure}

\newpage 
\section{\label{AnnexeB}Annexe B : Diagramme des observateurs / observés}
